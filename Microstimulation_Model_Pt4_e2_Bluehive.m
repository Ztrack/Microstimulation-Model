...
a = I0_Total(Neuron_Motion(1,:),:); a = a(:); a = sort(a,'descend');

for i = 1:45
    b(i) = find(I0_Total == a(i),1);
end
[Neuron_Target(1,:),Neuron_Target(2,:)] = ind2sub(size(I0_Soma_Neurons), b);
ElectrodeNo = unique(Neuron_Target(2,:)); % Activate all electrodes, we want to activate every motion neuron
...
%%
Neuron_Target(3,:) = nanmean(Neuron_Motion_RB(:,Neuron_Target(1,:)));

Stim_Electrode = []; % Electrode #
Stim_Electrode(:,1) = Neuron_Target(2,:); % Electrode #
Stim_Electrode(:,2) = Neuron_Target(3,:).*2; % Current intensity * 2 = Chronaxi
