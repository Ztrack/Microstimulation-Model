...
NumTrials = 100; % Number of trials per poisson process
...
output = NaN(length(units),NumNeurons,simulation); % Logical matrix denoting sucess (1) or failure (0) to spike in any given trial
...
for i = 1:length(units)
    ...
    for ii = 1:NumNeurons
        output(i,ii,iii) = Simple_PoissonGen2(lambda, dt, NumTrials,simulations,bpct);
    end
end
